...
def _build_per_row(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for prompt, response, simple in zip(
        df["prompt"].tolist(),
        df["responses"].tolist(),
...
def _aggregate(df: pd.DataFrame, model_name: str) -> pd.DataFrame:
    ...
    for sym in TARGET_SG_SYMBOLS:
        ...
        stats = wilson_ci_95(int(c1.astype(bool).sum()), int(len(c1)))
        grp_rows.append(
            {
                "model": model_name,
                ...
                "spacegroup_id": sid,
                "n_rows": int(len(g)),
                "n_claimed_eq_structure": stats["true_count"],
                "ratio_claimed_eq_structure": stats["ratio"],
                "ratio_claimed_eq_structure_ci_low": stats["ci_low"],
                "ratio_claimed_eq_structure_ci_high": stats["ci_high"],
            }
        )
...
def run(rl_path: Path, baseline_path: Path, data_dir: Path) -> None:
    ...
    out_df = pd.concat(
        [
            ...
            _aggregate(df_baseline, "spacegroup_thinking"),
        ],
        ignore_index=True,
    )
