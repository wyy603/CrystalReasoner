...
class FSDPSFTTrainer:
    ...
    def _build_model_optimizer(self):
        ...
        with init_context():
            ...
            #new_vocab_size = len(self.tokenizer) wyy:tokenizer
            #self.model.resize_token_embeddings(new_vocab_size) wyy:tokenizer
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True, n_micro_batches=1):
        ...
        task_mask = batch.pop("task_mask")[:, 1:].reshape(-1).to(self.device_name)
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                task_loss = loss * task_mask.to(loss.device)
                ...
                task_loss = full_loss * task_mask.to(loss.device)

            valid_token_this_rank = torch.sum(loss_mask)
            valid_task_token_this_rank = torch.sum(task_mask)

            if self.config.data.balance_dp_token:
                torch.distributed.all_reduce(valid_token_this_rank)
                torch.distributed.all_reduce(valid_task_token_this_rank)
            ...
            task_loss = torch.sum(task_loss) / (valid_task_token_this_rank + 1e-8) * dp_size
            task_loss = task_loss / n_micro_batches  # normalize loss

            if do_backward:
                loss.backward()
            return loss, task_loss.detach()
    ...
    def training_step(self, batch: TensorDict):
        ...
        for micro_batch in micro_batches:
            loss, task_loss = self._compute_loss_and_backward(batch=micro_batch, n_micro_batches=n_micro_batches)
            step_loss += loss.item()
            step_task_loss += task_loss.item()
        ...
        step_task_loss = torch.tensor(step_task_loss).to(self.device_name)
        ...
        if is_cuda_available:
            ...
            torch.distributed.all_reduce(step_task_loss, op=torch.distributed.ReduceOp.AVG)
        elif is_npu_available:
            torch.distributed.all_reduce(step_loss)
            step_loss /= self.device_mesh.size(0)
            torch.distributed.all_reduce(step_task_loss)
            step_task_loss /= self.device_mesh.size(0)
        return {
            "train/loss": step_loss.detach().item(),
            "train/task_loss": step_task_loss.detach().item(),
    ...
    def validation_step(self, batch: TensorDict):
        ...
        with torch.no_grad():
            loss, task_loss = self._compute_loss_and_backward(batch, do_backward=False)
            if is_cuda_available:
                torch.distributed.all_reduce(loss, op=torch.distributed.ReduceOp.AVG)
                torch.distributed.all_reduce(task_loss, op=torch.distributed.ReduceOp.AVG)
            elif is_npu_available:
                torch.distributed.all_reduce(loss)
                torch.distributed.all_reduce(task_loss)
                loss /= self.device_mesh.size(0)
                task_loss /= self.device_mesh.size(0)
            ...
            for step_in_epoch, data in enumerate(
                ...
                if is_last_step or (self.config.trainer.test_freq > 0 and is_valid_step):
                    ...
                    for val_data in self.val_dataloader:
                        ...
                        val_loss, val_task_loss = self.validation_step(val_data)
                        val_losses.append(val_loss)
                        val_task_losses.append(val_task_loss)
                    if rank == 0:
                        val_loss = torch.mean(torch.stack(val_losses))
                        val_task_loss = torch.mean(torch.stack(val_task_losses))
                        metric = {
                            "val/loss": val_loss.detach().item(),
                            "val/task_loss": val_task_loss.detach().item()
                        }
                        tracking.log(data=metric, step=global_step)
...
if __name__ == "__main__":
    ...
    #     print(f"Rank {os.environ.get('RANK')} waiting for debugpy client at 0.0.0.0:5678...")
