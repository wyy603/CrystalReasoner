...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        #print(f"len of val = {len(self.val_dataloader)}, {self.val_dataloader.dataset.split_name}, {len(self.val_dataloader.dataset.split['test'])}")
        ...
        for key_info, lst in reward_extra_infos_dict.items():
            assert len(lst) == 0 or len(lst) == len(sample_scores), f"{key_info}: {len(lst)=}, {len(sample_scores)=}, lst={lst}, sample_scores={sample_scores}"
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        if self.config.reward_model.launch_reward_fn_async:
                            future_reward = compute_reward_async.remote(
                                data=batch, config=self.config, tokenizer=self.tokenizer, reward_fn=self.reward_fn
                            )
                        else:
