...
START_STEP=${1:-1}
MERGED_DIR="checkpoints_merged/no_thinking"

source .venv/bin/activate

# ----------------------- [1/4] SFT -----------------------
if [ "$START_STEP" -le 1 ]; then
    echo "======================= [1/4] python scripts/run.py sft_no_thinking"
    python scripts/run.py sft_no_thinking --level=debug
else
    echo ">>> Skip Step [1/4]"
fi
...
if [ "$START_STEP" -le 2 ]; then
    ...
    python scripts/run.py merge \
      --path checkpoints/no_thinking/global_step_1514/ \
